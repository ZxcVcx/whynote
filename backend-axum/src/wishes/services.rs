...
pub async fn wish_new(db: Database, wish_new: WishNew) -> GqlResult<Wish> {
    ...
    let exist_document = coll
        .find_one(doc! {"user_id": &wish_new.user_id, "aphorism": &wish_new.aphorism})
        .await?;
    if let Some(_document) = exist_document {
        ...
        coll.insert_one(wish_new_document)
            .await
...
pub async fn wishes(db: Database, published: i32) -> GqlResult<Vec<Wish>> {
    ...
    // }
    match published.cmp(&0) {
        Ordering::Greater => { find_doc.insert("published", true); },
        Ordering::Less => { find_doc.insert("published", false); },
        Ordering::Equal => {},
    }
    let coll = db.collection::<Document>("wishes");
    let mut cursor = coll.find(find_doc).await?;
...
async fn one_wish(db: Database, match_doc: Document) -> GqlResult<Wish> {
    let coll = db.collection::<Document>("wishes");
    let mut cursor = coll
        .aggregate(vec![doc! {"$sample": {"size": 1}}, match_doc])
        .await?;
